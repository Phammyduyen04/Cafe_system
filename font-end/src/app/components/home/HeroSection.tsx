...
function HeroCarousel() {
  ...
  return (
    <div className="relative z-10 flex flex-col md:flex-row items-center justify-between w-full max-w-[1400px] mx-auto px-6 sm:px-10 md:px-16 pt-24 pb-28 md:pb-32">
      {/* Left text */}
      <div
        className="flex flex-col gap-6 md:gap-8 text-white max-w-[520px] w-full text-center md:text-left items-center md:items-start"
      >
      ...
      <div className="hidden md:block absolute right-0 top-0 bottom-0 w-[45%] pointer-events-none">
        <img
          ...
          src={slide.image}
          alt="Coffee"
          className="w-full h-full object-cover opacity-60 rounded-l-full"
          style={{
      ...
      <style>{`@keyframes fadeIn { from { opacity: 0 } to { opacity: 0.6 } }`}</style>
    </div>
  );
}
